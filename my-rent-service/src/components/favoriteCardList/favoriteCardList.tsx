            ...
            {Array.from(cities,(city)=>{
                return <div className="favorites__locations-items">
                    ...
                    <div className="favorites__places">
                        {Array.from(favoritesList.filter((f)=>f.city.name==city),(item)=><FavoriteCard key={item.id} id={item.id} title={item.title} type={item.type} price={item.price} isPremium={item.isPremium} previewImage={item.previewImage} rating={item.rating} location={item.city.name}/>)}
                    </div>
                </div>
            })}
        </div>
